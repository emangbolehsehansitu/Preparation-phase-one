...
function travelingIndonesia(arr, emoney) {
  ...
  for (let travel of arr) {
      ...
      // Loop untuk memisahkan nama dan perjalanan
      while (index < travel.length) {
          if (travel[index] === '-') {
              index++;
              continue;
          }
          if (!name) {
              while (index < travel.length && travel[index] !== '-') {
                  name += travel[index];
                  index++;
              }
          } else {
              let transport = '';
              while (index < travel.length && travel[index] !== '-') {
                  transport += travel[index];
                  index++;
              }
              journey.push(transport);
          }
      }

      let totalCost = 0;

      for (let transport of journey) {
          const cost = transportCosts[transport];
          if (cost) {
              const discount = cost * discountRates[emoney];
              totalCost += cost - discount;
          }
      }

      const departureCity = journey[0];
      const destinationCity = journey[journey.length - 1];

      hasil.push({
          name: name,
          departureCity: departureCity,
          destinationCity: destinationCity,
          transport: journey.join(', '),
          totalCost: totalCost,
      });
  }
